...
## 进阶设计：多尺度
对复合物进行层级化，多尺度的建模，我的创新点是对配体分子引入motif表示，对原子引入残基表示，然后在ESA的中间层（例如，在第3层之后），插入一个池化模块。这个模块将属于同一个官能团的所有原子的表示（Node representations）通过一个可学习的注意力机制或简单的平均/最大池化，聚合成一个官能团级别的表示，构建一个新的、更粗粒度的图。图的节点是官能团，边代表官能团之间的相互作用，边的特征可以由构成官能团的原子间边的特征聚合得到，在这个官能团图上，再运行几层ESA（MAB+SAB）这几层学习的是官能团之间的相互作用。
关于构建多尺度图：小分子用motif表示（RDkit提前处理），蛋白质用残基表示。